def it_was_ok
  ...
  # Movie.where('score < ? AND score > ?', 3, 2).select(:id, :title, :score)
end
...
def harrison_ford
  ...
  Movie.joins(:actors).where('actors.name = ?', 'Harrison Ford').select(:id, :title).where('castings.ord > ?', 1)
  # Movie.joins(:actors).where('actors.name = ?', 'Harrison Ford').select(:id, :title).where('castings.ord > ?', 1)
  # .where.not(castings_movies: {ord: 1})
